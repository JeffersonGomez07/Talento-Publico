const idAplicacion = localStorage.getItem("id_aplicacion");

// Función para llenar los elementos con los datos del usuario
const llenarDatosAplicacion = async () => {
  const aplicacion = await obtener_aplicacion_por_id(idAplicacion); // Utiliza la función que definiste antes
  ...
  if (aplicacion) {
    ...
    const primerExperiencia = aplicacion.experiencias[0];
    const segundoExperiencia = aplicacion.experiencias[1];
    const tercerExperiencia = aplicacion.experiencias[2];
